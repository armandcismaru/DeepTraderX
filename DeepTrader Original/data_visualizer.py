...
def box_plots():
    ...
    labels = [ "DeepTrader", "DeepTrader",
               "DeepTrader", "DeepTrader", "DeepTrader", "DeepTrader", "DeepTrader", "SNIPER", "GDX", "AA", "GIVEAWAY", "ZIC", "ZIP", "SHAVER"]
    ...
    results["Test Number"] = [x % 7 for x in range(len(labels)) for y in range(len(dtr_results[0]))]
    results["Trader"] = [labels[x] for x in range(len(labels)) for y in range(len(dtr_results[0]))]
    ...
    for opt in range(7):
        ...
        grid = sns.boxplot(data=df, x="Trader", y="Profit Per Trader", color= ["blue","red"], orient='v',
                        palette='bright', hue="Trader", width=0.5)
...
def loss_train():
    ...
    loss = [0.2514,  0.1702, 0.0970, 0.0428,
            0.0143,  0.0057, 0.0043,  0.0038, 0.0032,  0.0026,  0.0022, 0.0019, 0.0017,  0.0016, 0.0015,  0.0014,  0.0014,  0.0013, 0.0012, 0.0012]
